...
def cli(src: Path, out: Path, ext: str, ai_file: Path, dry_run: bool, verbose: bool) -> None:
...
def _perform_extraction(src_path: Path, out_path: Path, ai_file_path: Path, 
                       extensions: List[str], dry_run: bool, verbose: bool) -> None:
    ...
    try:
            ...
            try:
                ...
                if tikz_blocks:
                    if dry_run:
                        ...
                        metadata = _simulate_block_metadata(tikz_blocks, file_path, out_path)
                    else:
                        # Actually write the blocks using core extractor function
                        metadata = extractor.write_extracted_blocks(tikz_blocks, file_path, out_path)
                    ...
                    if verbose:
                        ...
                        for i, block_meta in enumerate(metadata, 1):
                            if dry_run:
                                click.echo(f"    Would create: {Path(block_meta['out_path']).name}")
                            else:
                                click.echo(f"    Created: {Path(block_meta['out_path']).name}")
                else:
                    if verbose:
        ...
        click.echo()
        _display_extraction_summary(processed_files, len(all_metadata), skipped_files, 
                                   error_files, dry_run, verbose)
        ...
        if all_metadata:
            if dry_run:
                ...
                if verbose:
                    click.echo(f"  Would contain {len(all_metadata)} TikZ blocks from {processed_files} files")
            else:
                try:
                    extractor.build_ai_context(all_metadata, ai_file_path)
                    click.echo(f"Created AI context file: {ai_file_path}")
                    if verbose:
                        click.echo(f"  Contains {len(all_metadata)} TikZ blocks from {processed_files} files")
                except (IOError, OSError) as e:
                    click.echo(f"Warning: Failed to create AI context file: {e}", err=True)
        else:
            if not dry_run:
        ...
        if processed_files == 0 and skipped_files == 0:
            click.echo("\nSuggestions:")
            click.echo("- Check if the source directory contains files with the specified extensions")
            ...
            click.echo("Make sure your files contain \\begin{tikzpicture}...\\end{tikzpicture} blocks.")
            
    except Exception as e:
        raise click.ClickException(f"Extraction failed: {e}")


def _display_extraction_summary(processed_files: int, total_blocks: int, skipped_files: int,
                               error_files: List[Tuple[Path, str]], dry_run: bool, verbose: bool) -> None:
    ...
    if total_blocks > 0:
        ...
        click.echo(f"✓ Successfully processed {processed_files} files (no TikZ blocks found)")
...
def _simulate_block_metadata(blocks: List[str], src_path: Path, out_dir: Path) -> List[Dict[str, any]]:
