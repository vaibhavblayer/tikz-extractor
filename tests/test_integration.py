...
class TestExtractFromDirectory:
    ...
    def test_extract_from_directory_multiple_blocks_per_file(self, test_file_structure):
        ...
        multiple_tikz_metadata = [
            ...
            if 'multiple_tikz.tex' in meta['source']
        ]
        ...
        assert multiple_tikz_metadata[0]['content'] != multiple_tikz_metadata[1]['content']
    ...
    def test_extract_from_directory_nested_files(self, test_file_structure):
        ...
        nested_sources = [
            meta['source'] for meta in metadata
            if 'nested' in meta['source']
        ]
    ...
    def test_extract_from_directory_specific_extensions(self, test_file_structure):
        ...
        md_sources = [
            meta for meta in metadata if meta['source'].endswith('.md')]
        py_sources = [
            meta for meta in metadata if meta['source'].endswith('.py')]
    ...
    def test_extract_from_directory_error_handling(self, test_file_structure, binary_file):
...
class TestCompleteWorkflow:
    ...
    def test_workflow_metadata_accuracy(self, test_file_structure):
        ...
        for meta in metadata:
            ...
            expected_filename = f"{extractor.sanitize_name(source_path)}__tikz{meta['index']}.tex"
    ...
    def test_workflow_with_different_source_structures(self, temp_dir):
        ...
        (temp_dir / 'root.tex').write_text(r'''
        \begin{tikzpicture}
        \draw (0,0) -- (1,1);
        \end{tikzpicture}
        ''', encoding='utf-8')

        (temp_dir / 'level1' / 'mid.tex').write_text(r'''
        \begin{tikzpicture}
        \node {Middle};
        \end{tikzpicture}
        ''', encoding='utf-8')

        (temp_dir / 'level1' / 'level2' / 'level3' / 'deep.tex').write_text(r'''
        \begin{tikzpicture}
        \draw (0,0) circle (1);
        \end{tikzpicture}
        ''', encoding='utf-8')
