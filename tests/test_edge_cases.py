...
class TestEmptyDirectoriesAndFiles:
    ...
    def test_extract_from_directory_with_empty_files(self, temp_dir):
        ...
        out_dir = temp_dir / 'output'
        metadata = extractor.extract_from_directory(temp_dir, out_dir, ['.tex', '.md', '.py'])
    ...
    def test_extract_from_files_without_tikz(self, temp_dir):
        ...
        (temp_dir / 'latex_no_tikz.tex').write_text(r'''
...
class TestErrorRecovery:
    ...
    def test_extract_with_encoding_errors(self, temp_dir, binary_file):
        ...
        (temp_dir / 'good.tex').write_text(r'''
        \begin{tikzpicture}
        \draw (0,0) -- (1,1);
        \end{tikzpicture}
        ''', encoding='utf-8')
    ...
    def test_extract_with_mixed_file_errors(self, temp_dir):
        ...
        (temp_dir / 'good1.tex').write_text(r'\begin{tikzpicture}\draw (0,0) -- (1,1);\end{tikzpicture}', encoding='utf-8')
        (temp_dir / 'good2.tex').write_text(r'\begin{tikzpicture}\draw (0,0) circle (1);\end{tikzpicture}', encoding='utf-8')
    ...
    def test_cli_error_recovery_with_verbose(self, temp_dir):
        ...
        (temp_dir / 'good.tex').write_text(r'\begin{tikzpicture}\draw (0,0) -- (1,1);\end{tikzpicture}', encoding='utf-8')
...
class TestLargeFilesAndDirectories:
    ...
    def test_extract_from_file_with_many_tikz_blocks(self, temp_dir):
        ...
        content_parts = []
        for i in range(50):
            content_parts.append(f'\\begin{{tikzpicture}}\\node {{Block {i}}}; \\end{{tikzpicture}}')
